...
//        $priceGroup = StoreOrder::getOrderPriceGroup($cartInfo);
    ...
    public function pay(Request $request)
    {
        ...
        switch ($order['pay_type']) {
            ...
            case 'yue':
                ...
                break;
            case 'payh5code':
                if (StoreOrder::payh5code($order['order_id'], $request->uid()))
                    return app('json')->status('success', '收款码支付成功');
                else {
                    $error = StoreOrder::getErrorInfo();
                    return app('json')->fail(is_array($error) && isset($error['msg']) ? $error['msg'] : $error);
                }
     ...
     * @throws \think\exception\DbException
     */
    public  function order_issale(Request $request)
    {
        list($id) = UtilService::postMore([['id',0]],$request, true);
        if(!$id) return app('json')->fail('参数错误');
        if (StoreOrder::isaleOrder($id, $request->uid()))
