...
class StoreOrder extends BaseModel
{
    ...
    protected static $payType = ['weixin' => '微信支付', 'yue' => '余额支付', 'offline' => '线下支付', 'payh5code'=>'扫码支付'];
...
//        $totalPrice = bcadd($totalPrice,$storePostage,2);
     ...
     * @throws \think\exception\DbException
     */
    public static function isaleOrder($order_id, $uid)
    {
        $order = self::where('id', $order_id)->where('uid', $uid)->find();
        if (!$order) return self::setErrorInfo('没有查到此订单');
        Db::startTrans();
        try {
            $product_id = Db::name('store_order_cart_info')->where('oid',$order_id)->value('product_id');
            Db::name('store_product')->where('id',$product_id)->update(['uuid'=>0,'order_id'=>0]);
            $order->status = 3;
            $order->pay_type = 'payh5code';
            if ($order->save()) {
                Db::commit();
                return true;
            } else
...
//            }
    ...
    public function ckeckOrderUnPay(){
