...
//        if($salesOrder) $baseOrder = $salesOrder == 'desc' ? 'sales DESC' : 'sales ASC';//真实销量
    ...
    public static function getPaycode($id){
        ...
        if ($uuid) {
            $url = DB::name('user')->where('uid', $uuid)->value('h5_pay_code');
        }else{
            $url = sys_config('h5_pay_code');
        }
        return $url;
    }

    public static function paycodebyorder($id){

        $product_id = DB::name('store_order_cart_info')->where('oid', $id)->value('product_id');
        $uuid = self::where('id', $product_id)->value('uuid');
        if ($uuid) {
            $url = DB::name('user')->where('uid', $uuid)->value('h5_pay_code');
        }else{
            $url = sys_config('h5_pay_code');
