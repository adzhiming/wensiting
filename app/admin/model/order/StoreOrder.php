...
class StoreOrder extends BaseModel
{
    ...
    public static function orderCount()
    {
        ...
        $data['sale'] = self::where('status',  9)->where('paid', 1)->count();
        $data['is_sale'] = self::where('is_sale','>', 0)->count();
        return $data;
    }
        ...
        if ($where['export'] == 1) {
            ...
            foreach ($list as $index => $item) {
                ...
                if ($item['pay_type'] == 'weixin') {
                    ...
                    $payType = '线下支付';
                } elseif ($item['pay_type'] == 'payh5code') {
                    $payType = '收款码支付';
                } else {
                    $payType = '其他支付';
                }
...
//        $model = $model->where('combination_id',0);
        ...
        if (isset($where['pay_type'])) {
            switch ($where['pay_type']) {
                ...
                case 4:
                    $model = $model->where($aler . 'pay_type', 'payh5code');
                    break;
            }
        }
        if (isset($where['type'])) {
...
//                    $model = $model->where($aler.'combination_id',">",0)->where($aler.'pink_id',">",0);
                ...
                case 4:
                    ...
                    break;
                case 5:
                    $model = $model->where($aler . 'is_sale', '>', 0);
                    break;
            }
        }
...
//        if($where['is_del'] != '' && $where['is_del'] != -1) $model = $model->where('is_del',$where['is_del']);
    ...
    public static function getEchartsOrder($where, $limit = 20)
    {
        ...
        $bing_xdata = ['微信支付', '余额支付', '收款码支付','其他支付'];
        ...
        foreach ($bingpaytype as $key => $item) {
            ...
            } else if ($item['pay_type'] == 'payh5code') {
    ...
    public static function payTypeCount()
    {
        ...
        $where['pay_type'] = 4;
        $payh5code = self::getOrderWhere($where, new self)->count();
        return compact('weixin', 'yue', 'offline','payh5code');
    }
}