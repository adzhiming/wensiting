...
class StoreOrder extends AuthController
{
    ...
    public function edit($id)
    {
        ...
        // $f[] = Form::selectOne('status','订单状态',$product->getData('status'))->setOptions(function(){
        //     $status = array();
        //     // 订单状态（-1 : 申请退款 -2 : 退货成功 0：待发货；1：待收货；2：已收货；3：待评价；-1：已退款）
        //     $status[] = ['value'=>-2,'label'=>'退货成功'];
        //     $status[] = ['value'=>-1,'label'=>'申请退款/已退款'];
        //     $status[] = ['value'=>0,'label'=>'待发货'];
        //     $status[] = ['value'=>1,'label'=>'待收货'];
        //     $status[] = ['value'=>2,'label'=>'已收货'];
        //     $status[] = ['value'=>3,'label'=>'待评价'];
        //     return $status;
        // });
//        $f[] = Form::radio('status','状态',$product->getData('status'))->options([['label'=>'开启','value'=>1],['label'=>'关闭','value'=>0]]);
    ...
    public function order_sale($id){
        ...
        if($res['code'] == 0){
            ...
            return Json::fail(StoreOrderModel::getErrorInfo($res['msg']));
        }
    }
